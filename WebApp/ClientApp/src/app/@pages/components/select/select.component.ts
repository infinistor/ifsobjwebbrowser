...
export class pgSelectComponent implements OnInit, AfterContentInit, AfterContentChecked, ControlValueAccessor {
  ...
  addOption = (option: pgOptionComponent) => {
  ...
  unSelectMultipleOption = (option: pgOptionComponent, $event?: any, emitChange = true) => {
  ...
  closeDropDown(): void {
    ...
    this.onTouched();
    if (this.Multiple) {
      this._renderer.removeStyle(this.searchInput.nativeElement, 'width');
    }
    this.clearSearchText();
    this.Open = false;
  }
