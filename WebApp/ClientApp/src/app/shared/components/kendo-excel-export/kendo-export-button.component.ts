...
import { ExcelExportComponent, Workbook, WorkbookSheet, WorkbookSheetColumn, WorkbookSheetRow, WorkbookSheetRowCell } from "@progress/kendo-angular-excel-export";
...
@Component({
	selector: 'common-kendo-export-button',
...
export class CommonKendoExportButtonComponent implements OnInit {
	...
	exportToExcel(exportMedia: string) {
								...
								this.grid.columns.map((column) => {
									...
									if(column.pipe) {
										if(column.customTemplate) {
											// @ts-ignore
											cells.push({ value: dynamicPipe.transform(row, column.pipe, column.customTemplate, column.field, false), color: color });
										}
										else {
											// @ts-ignore
											cells.push({ value: dynamicPipe.transform(row, column.pipe, column.pipeEnumTranslateClass, column.field, false), color: color });
										}
									}
									else {
										// @ts-ignore
										cells.push({ value: row[column.field], color: color });
									}
								});
								rows.push({ cells: cells});
							});
