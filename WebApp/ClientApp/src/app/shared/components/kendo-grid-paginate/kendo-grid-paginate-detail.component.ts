...
import { RowArgs, PageChangeEvent, SelectionEvent, CellClickEvent } from '@progress/kendo-angular-grid';
...
@Component({
		selector: 'common-kendo-grid-paginate-detail',
...
export class CommonKendoPaginateDetailGridComponent implements OnInit, IGridComponent {
	 ...
	 */
	@Output() pageChangeEmitter: EventEmitter<PageChangeEvent> = new EventEmitter<PageChangeEvent>();
	...
	*/
	public ngDoCheck(): void {
		if (this.keyValueDiffers) {
			// 검색 오브젝트의 변화를 감지
			let searchChanges = this.KeyValueDiffer.diff(this.detailGrid.commonSearch);
	...
	*/
	...
	public GetKeyValue(rowData: any) {
		...
		if (rowData !== null) {
			for (var index = 0; index < this.detailGrid.keyColumnNames.length; index++) {
	...
	*/
	 ...
	 * @param total 전체 개수
	 */
	public updateGridView(data: any[], total: number): void {
		this.detailGrid.items = data;
		this.detailGrid.gridView = { data, total };
	...
	selectByIndex(index: number): any {
		...
		if(this.detailGrid.selectedItems) {
			this.detailGrid.selectedItems.map((item, index) => {
