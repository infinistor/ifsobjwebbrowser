...
import { RowArgs, PageChangeEvent, SelectionEvent, CellClickEvent, DetailExpandEvent, DetailCollapseEvent, GridComponent } from '@progress/kendo-angular-grid';
...
@Component({
		selector: 'common-kendo-grid-paginate',
...
export class CommonKendoGridPaginateComponent implements OnInit, IGridComponent {
	/**
	...
	*/
	...
	public static propertyName: string = '';
	...
	*/
	constructor(
		...
		public messageService: MessageNotificationService,
	...
	*/
	public ngDoCheck(): void {
		if (this.keyValueDiffers) {
			// 검색 오브젝트의 변화를 감지
			let searchChanges = this.KeyValueDiffer.diff(this.grid.commonSearch);
	...
	*/
	public setGridHeight(): void {
		this.grid.height = this.grid.isUseFixedHeight ? this.grid.height : window.innerHeight - 190 - this.grid.flexingSize;
	}
	...
	*/
	...
	public onResize(event: any): void {
		//this.grid.height = this.grid.isUseFixedHeight ? this.grid.height : event.target.innerHeight - 190 - this.grid.flexingSize;
		this.setGridHeight();
	}
	...
	*/
	...
	private loadConfig(): void {
		...
		let config = this.gridColumnConfigService.getConfigByGrid(this.grid);
	...
	* @param searchKeyword
	*/
	public getList<T>(apiUrl: string, skip: number, countPerPage: number, searchFields: Array<string> = [], searchKeyword = ''): void {
		let param = {
			skip: skip,
	...
	* @param data
	*/
	public bindList<T>(currSkip: number, response: ResponseList<T>): void {
	...
	*/
	 ...
	 * @param total 전체 개수
	 */
	public updateGridView(data: any[], total: number = 0): void {
		this.grid.items = data;
		this.grid.gridView = { data, total };
	...
	*/
	...
	public getSelectionKey(context: RowArgs): string {
		...
		// 해당 행 데이터의 키 값을 가져온다.
		const value = $this.GetKeyValue(context.dataItem);;
	 ...
	 * @param sorts 소트정보
	 */
	public sortChange(sorts: SortDescriptor[]) {
	 ...
	 * @param event
	 */
	public detailExpand(event: DetailExpandEvent) {
	 ...
	 * @param index
	 */
	public collapseRow(index: number) {
		if(this.commonGrid) {
	 ...
	 * @param event 선택 여부
	 */
	public onChangeColumnCheckAll(field: string, event: any) {
		this.checkColumnAllChangedEmitter.emit({ field: field, checked: event});
	}
	 ...
	 * @param event 선택 여부
	 */
	public onChangeColumnCheck(item: any, field: string, event: any) {
		item[field] = event;
		this.checkColumnChangedEmitter.emit({ data: item, field: field, checked: event});
	}
	...
	selectByIndex(index: number): any {
		...
		if(this.grid.selectedItems) {
			this.grid.selectedItems.map((item, index) => {
				deselectedRows.push({dataItem: item, index: index});
			});
		}
		...
		if(this.grid.items && index >= 0 && index < this.grid.items.length) {
			...
			if(selctedKeyValue) {
				...
				if(this.grid.selectedItems) {
					this.grid.selectedItems.map((item, index) => {
						selectedRows.push({dataItem: item, index: index});
					});
				}
				this.selectionChangeEmiiter.emit({selectedRows: selectedRows, deselectedRows: deselectedRows, ctrlKey: false, shiftKey: false});
