...
@Component({
	selector: 'common-kendo-grid-virtual',
...
export class CommonKendoGridVirtualComponent implements OnInit, IGridComponent {
	@Input()
	get grid(): Grid {
		return this._grid;
	}
	...
	calculatePageSize(gridHeight: number, rowHeight: number, headerHeight: number, multipleNumberOfNextPrevious: number = 2) {
		const rowsHeightWithoutHeader = gridHeight - headerHeight;
		const rowCount = Math.ceil(rowsHeightWithoutHeader / rowHeight);

		// 페이지 사이즈 조정을 사용할경우
		if (this.grid.isUseCalcuratePageSize) return rowCount * multipleNumberOfNextPrevious;
		// 페이지 사이즈 조정을 사용하지 경우
		else return this.grid.pageSize;
	}
	...
	private takeItems(): Observable<any[]> {
		...
		if (currentView) {
			return Observable.of(currentView);
		}
	...
	private emptyArray(pageSize: number, skip: number, totalCount: number) {
		...
		const subscription = this.takeItems()
			.pipe(
				catchError((err: HttpErrorResponse) => {
					...
					return Observable.of(emptyArrayForError);
				})
			)
			.subscribe((items: any[]) => {
