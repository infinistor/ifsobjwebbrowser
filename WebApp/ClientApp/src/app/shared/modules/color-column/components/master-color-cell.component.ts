...
@Component({
    selector: 'master-color-cell',
    templateUrl: './master-color-cell.component.html',
    styleUrls: ['./master-color-cell.component.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush,
})
export class MasterColorCellComponent implements OnInit, OnChanges {
    @Input() colorColumn: ColorColumn;
    @Input() rowData: any;
    @Input() countOfColor = 1;
    @Input() ignoreDetail = false;
    ...
    private fillColor() {
        ...
        const cellColor = this.colorColumnService.createCellColor(this.rowData, this.colorColumn, this.ignoreDetail);
