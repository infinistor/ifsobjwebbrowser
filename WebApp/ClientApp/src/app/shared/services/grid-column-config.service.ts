...
export class GridColumnConfigService {
	...
	private getConfigsFromStorage(): GridColumnsConfig[] {
		let configs = localStorage.getItem(this.STORAGE_KEY);
		if(configs) {
	...
	setConfig(config: GridColumnsConfig): void {
		...
		let found = configs.find(item => item.gridPath === config.gridPath);
		if (found) {
	...
	setConfigByGrid(grid: Grid): void {
		let gridPath = grid.getGridPath();
		let hiddenColumnFields = grid.getHiddenColumnFields();
